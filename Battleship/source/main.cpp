...
int main(int argc, char const *argv[])
{
    ...
    for (int i = 0; i < QT; ++i)// esse for vai até o numero de puzzles decidido pelo usuario
    {
        ...
        //a ideia de passar 18 é para sempre criar uma matriz maior que o limite 16 para poder trabalhar com as bordas dos barcos sem se preocupar com 
        //o uso de memoria de indevido
	    criar_tabuleiro(18, 18, jogo,X , Y);//criando tabuleiro

	    plotar_bt(jogo, X, Y);//plotando o primeiro barco o batlehsip

	    plotar_destroyer(jogo, X, Y);//plotando os 2 destroyer
        plotar_destroyer(jogo, X, Y);

        plotar_cruser(jogo, X, Y);//plotando os 3 crusador
        plotar_cruser(jogo, X, Y);
        plotar_cruser(jogo, X, Y);

        plotar_sub(jogo, X, Y);//plotando os 4 submarino
        plotar_sub(jogo, X, Y);
        plotar_sub(jogo, X, Y);
        plotar_sub(jogo, X, Y);

        print_tabu_terminal(jogo, X, Y);//printando no terminal os tabuleiros
